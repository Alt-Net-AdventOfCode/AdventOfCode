...
namespace AdventCalendar2018
{
    public static class Day20
    {
        private static void MainDay20()
        {
            ...
            var shift = new Dictionary<char, (int dx, int dy)>
            {
            ...
            foreach (var car in line)
            {
                switch (car)
                {
                    case ')':
                        ...
                        // we drop our starting point
                        history.Pop();
                        curDist = distances[cursor];
                    ...
                    default:
                        curDist++;
                        cursor = cursor.Move(shift[car]);
                        UpdateDistance(cursor, curDist, distances);
                        length.Peek()[0]++;
                        break;
                }
            }

            Console.WriteLine($"Furthest room dist : {targetLength}");
            Console.WriteLine($"{distances.Values.Count(i => i>=limit)} rooms beyond {limit}");
        }

        private static void UpdateDistance(Coord cursor, int curDist, Dictionary<Coord,int> distances)
        {
            if (distances.ContainsKey(cursor))
            {
                distances[cursor] = Math.Min(curDist, distances[cursor]);
            }
            else
            {
                distances[cursor] = curDist;
        ...
        private class Coord
        {
            ...
            public Coord Move((int dx, int dy) offset)
            {
