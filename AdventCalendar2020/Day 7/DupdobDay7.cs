...
namespace AdventCalendar2020.Day_7
{
    public class DupdobDay7
    {
        public void Parse(string input = Input)
        {
            var masterRegexp = new Regex("(.+) bags contain (.+).", RegexOptions.Compiled);
            var subRegexp = new Regex(" ?(\\d?) (.+) bags?\\.?", RegexOptions.Compiled);
            foreach (var line in input.Split('\n'))
            {
                var match = masterRegexp.Match(line);
                if (!match.Success)
                {
                ...
                if (content!="no other bags")
                {
                    ...
                    {
                        var sub = subRegexp.Match(block);
                        if (!sub.Success)
                        {
                            Console.WriteLine("Failed to parse {0)", block);
                            continue;
                        }

                        var count = int.Parse(sub.Groups[1].Value);
                        var subName = sub.Groups[2].Value;
                        rules[subName] = count;
                    }}
        ...
        {
            var detailedContents = new Dictionary<string, Dictionary<string, int>>();
            foreach (var entry in constraints)
            { 
                Expand(entry.Key);
            }

            return expanded.Count(t => t.Value.ContainsKey("shiny gold"));
        }

        private Dictionary<string, int> Expand(string bag)
        {
            if (expanded.ContainsKey(bag))
            {
                return expanded[bag];
            }
            var entryValue = constraints[bag];
            var map = new Dictionary<string, int>();

            foreach (var (key, value) in entryValue)
            {
                var content = Expand(key);
        ...
        public object GiveAnswer2()
        {
            // 34145 too low
            // 52820 too low
            return expanded["shiny gold"].Values.Sum();
        }
        
        private readonly Dictionary<string, Dictionary<string, int>> constraints = new Dictionary<string, Dictionary<string, int>>();

        private readonly Dictionary<string, Dictionary<string, int>> expanded =
            new Dictionary<string, Dictionary<string, int>>();
        private const string Input = @"posh brown bags contain 5 dim coral bags, 1 plaid blue bag, 2 faded bronze bags, 2 light black bags.
