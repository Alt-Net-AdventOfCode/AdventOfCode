...
namespace AdventCalendar2020.Day_4
{
    public class DupdobDay4
    {
        public void Parse(string input = Input)
        {
            foreach (var passportFields in input.Split("\n\n"))
            {
                var pass = new Dictionary<string, string>();
                foreach (var line in passportFields.Split("\n"))    
                {
        ...
        public object GiveAnswer1()
        {
            ...
            {
                if (passport.Count >= fields.Length && fields.All(f => passport.ContainsKey(f)))
                {
                    legal++;
                }
            }
        ...
        public object GiveAnswer2()
        {
            var fields = new Dictionary<string, string>() {
                ...
                {"pid", "\\d{9}$"}};
            var legal = 0;
            var compiledFields = new Dictionary<string, Regex>();
            foreach (var entry in fields)
            {
                compiledFields[entry.Key] = new Regex(entry.Value, RegexOptions.Compiled);   
            }
            foreach (var passport in _passports)
            {
                if (passport.Count >= fields.Count && fields.All(f => passport.ContainsKey(f.Key)))
                {
                    var ok = true;
                    foreach (var field in compiledFields)
                    {
                        var passportField = passport[field.Key];
                        
                        if (!field.Value.Match(passportField).Success)
                        {
                            Console.WriteLine($"field {field.Key} did not match: {passportField}");
                            ok = false;
                            break;
                        }
        ...
        private List<Dictionary<string, string>> _passports = new List<Dictionary<string, string>>();
        private const string Input = @"byr:1985
