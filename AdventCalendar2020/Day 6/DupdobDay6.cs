...
namespace AdventCalendar2020.Day_6
{
    internal class DupdobDay6
    {
        ...
        public object? GiveAnswer1()
        {
            ...
            {
                var foundAnswers = new HashSet<char>();
                foreach (var answer in groupAnswers)
                {
                    foreach (var entry in answer)
                    {
                        foundAnswers.Add(entry);
                    }
                }

                count += foundAnswers.Count;
            }
            return count;
        }
        ...
        public object? GiveAnswer2()
        {
            ...
            foreach (var groupAnswers in _groupsAnswers)
            {
                var possibleAnswers = new HashSet<char>();
                for (var car = 'a'; car <= 'z'; car++)
                {
                    possibleAnswers.Add(car);
                }
                foreach (var answer in groupAnswers)
                {
                ...
                count += possibleAnswers.Count;
            }
            return count;        }

        private List<List<string>> _groupsAnswers = new List<List<string>>();
        private const string Input = @"gyrf
idg
