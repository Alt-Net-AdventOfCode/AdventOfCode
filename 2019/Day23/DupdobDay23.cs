...
namespace AdventCalendar2019.Day23
{
    public  class DupdobDay23
    {
        ...
        {
            var puzzle = new DupdobDay23[50];
            var network = new Dictionary<long, Queue<long>>();
            var runners = new List<Thread>();
            for (var i = 0; i < puzzle.Length; i++)
            {
                puzzle[i] = new DupdobDay23();
                puzzle[i].ParseInput();
            ...
            for (var i = 0; i < puzzle.Length; i++)
            {
                ...
                {
                    var index = (int) x;
                    var buffer = new List<long>(3);
                    puzzle[index].RunProgram(l =>
                    {
                        if (l == 0) return index;
                        var result = -1L;
                        lock(network)
                        {
                            if (network[index].Count == 0)
                            {
                                if (index == 0 && network[255].Count==2)
                                {
                                    var idle = true;
                                    for (var verify = 1; verify < 50; verify++)
                                    {
                            ...
                            result = network[index].Dequeue();
                        }

                        Console.WriteLine($"Computer {x}, read {result}.");
                        return result;
                    }, l =>
                    {
                        buffer.Add(l);
