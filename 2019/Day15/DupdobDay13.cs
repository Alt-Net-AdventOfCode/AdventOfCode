...
namespace AdventCalendar2019.Day15
{
    public  class DupdobDay15
    {
        ...
        {
            var puzzle = new DupdobDay15();
            puzzle.ParseInput();
            Console.WriteLine($"Answer 1: {puzzle.FindShortestPathToOxygen()}");
            Console.WriteLine($"Answer 2: {puzzle.MaxDistanceToOxygen()}");
        }

        private int FindShortestPathToOxygen()
        {
            var distance = ExploreShip();
            return distance;
        }
        ...
        private int ExploreShip()
        {
            _map = new Dictionary<(int x, int y), Cell>();
            (int x, int y) droid = (0, 0);
            var directions = new (int dx, int dy)[] {(0, -1),(0,1),(-1,0),(1,0)};
            var currentCell = new Cell(0, 0, null) {Type = 1};
            var lastDirection = 0;
            var mustStop = false;
            var step = 0;
            _newCell = null;
            _map[droid] = currentCell;
            
            long InputProvider(long index)
            {
                lastDirection = currentCell.NextUnvisitedDirection();
                if (lastDirection >= 0)
                {
                    return lastDirection+1;
                }
                else if (currentCell == _map[(0, 0)])
                {
                ...
                lastDirection = currentCell.ParentDirection;
            ...
            bool OutputHandler(long output)
            {
                ...
                var (dx, dy) = directions[lastDirection];
                var attemptedMove = (droid.x + dx, droid.y + dy);
                step++;
                if (!_map.TryGetValue(attemptedMove, out var newCell))
                {
                    newCell = new Cell(lastDirection, step, currentCell);
                    _map[attemptedMove] = newCell;
                    newCell.X = attemptedMove.Item1;
                    newCell.Y = attemptedMove.Item2;
                }
                else if (step < newCell.Distance)
                {
                    newCell.Distance = step;
                }
                
                currentCell.Neighbours[lastDirection] = newCell;
                switch (output)
                {
                    case 0:
                        // wall
                ...
                step = currentCell.Distance;
                return false;
            }

            RunProgram(InputProvider, OutputHandler);

            return _newCell.Distance;
        }

        private int MaxDistanceToOxygen()
        {
            // compute oxygen spread type
            var distanceOxygen = new Dictionary<(int x, int y), int>();
            ...
            foreach (var key in _map.Keys)
            {
                ...
                distanceOxygen[key] = int.MaxValue;
                visited.Add(key);
            }

            distanceOxygen[(_newCell.X, _newCell.Y)] = 0;
            for (; visited.Count>0;)
            {
                ...
                {
                    if (distanceOxygen[cell] < nearest)
                    {
                        next = cell;
                        nearest = distanceOxygen[cell];
                    }
                }

                visited.Remove(next);
                nearest++;
                foreach (var neighbour in _map[next].Neighbours)
                {
                    var coord = (neighbour.X, neighbour.Y);
                    if (neighbour.Type > 0 && distanceOxygen[coord] > nearest)
                    {
                        distanceOxygen[coord] = nearest;
                    }
                }
            }

            return distanceOxygen.Values.Max();
        }
        ...
        private class Cell
        {
            public readonly Cell[] Neighbours = new Cell[4];

            public int Type;
            public readonly int ParentDirection;
            public int Distance;
            public int X;
            public int Y;

            public Cell(int arrivingDirection, int distance, Cell currentCell)
            {
                this.Distance = distance;
                this.ParentDirection = arrivingDirection switch
                {
                    0 => 1,
                    1 => 0,
                    2 => 3,
                    3 => 2,
                    _ => this.ParentDirection
                };
                this.Neighbours[this.ParentDirection] = currentCell;
            }

            public int NextUnvisitedDirection()
            {
