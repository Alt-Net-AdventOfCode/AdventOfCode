    ...
    public class DupdobDay4 : DupdobDayWithTest
    {
        ...
        public override object GiveAnswer1()
        {
            ...
            foreach (var entry in _words)
            {
                ...
                foreach (var letter in line.Where(letter => letter is >= 'a' and <= 'z'))
                {
                    if (!hits.ContainsKey(letter))
                    {
                        hits[letter] = 1;
                    }
                    else
                ...
                var ordered = hits.OrderByDescending(e => e.Value*100+('a' - e.Key)).Take(5);
                var index = 0;
                foreach (var letter in ordered)
                {
                    if (match.Groups[3].Value[index] != letter.Key)
                    {
                        break;
                    }
